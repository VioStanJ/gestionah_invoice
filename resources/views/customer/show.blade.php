...
@section('action-button')
    <div class="row d-flex justify-content-end">
        @can('create invoice')
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-6">
                <div class="all-button-box">
                    <a href="{{ route('invoice.create',$info->id) }}" class="btn btn-xs btn-white btn-icon-only width-auto">
                        {{__('Create Invoice')}}
                    </a>
                </div>
            </div>
        ...
        @can('create proposal')
            <div class="col-xl-2 col-lg-3 col-md-4 col-sm-6 col-6">
                <div class="all-button-box">
                    <a href="{{ route('proposal.create',$info->id) }}" class="btn btn-xs btn-white btn-icon-only width-auto">
                        {{__('Create Proposal')}}
                    </a>
                </div>
            </div>
        ...
        @can('edit info')
            <div class="col-xl-1 col-lg-2 col-md-2 col-sm-6 col-6">
                <div class="all-button-box">
                    <a href="#" data-size="2xl" data-url="{{ route('info.edit',$info['id']) }}" data-ajax-popup="true" data-title="{{__('Edit info')}}" data-toggle="tooltip" data-original-title="{{__('Edit')}}" class="btn btn-xs btn-white btn-icon-only width-auto">
        ...
        @can('delete info')
            <div class="col-xl-1 col-lg-2 col-md-2 col-sm-6 col-6">
                <div class="all-button-box">
                    <a href="#" data-toggle="tooltip" data-original-title="{{__('Delete')}}" data-confirm="{{__('Are You Sure?').'|'.__('This action can not be undone. Do you want to continue?')}}" data-confirm-yes="document.getElementById('delete-form-{{ $info['id']}}').submit();" class="btn btn-xs btn-white bg-danger btn-icon-only width-auto">
                        <i class="fa fa-trash"></i>
                    </a>
                    {!! Form::open(['method' => 'DELETE', 'route' => ['info.destroy', $info['id']],'id'=>'delete-form-'.$info['id']]) !!}
                    {!! Form::close() !!}
                </div>
            </div>
        @endcan
...
@section('content')
    <div class="row">
        <div class="col-md-4 col-lg-4 col-xl-4">
            <div class="card pb-0 info-detail-box">
                <h3 class="small-title">{{__('info Info')}}</h3>
                <div class="p-4">
                    <h5 class="report-text gray-text mb-0">{{$info['name']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$customer['email']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['contact']}}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-4 col-lg-4 col-xl-4">
            <div class="card pb-0 info-detail-box">
                ...
                <div class="p-4">
                    <h5 class="report-text gray-text mb-0">{{$info['billing_name']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['billing_phone']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['billing_address']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['billing_zip']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['billing_city'].', '. $info['billing_state'] .', '.$info['billing_country']}}</h5>
                </div>
            </div>
        </div>
        <div class="col-md-4 col-lg-4 col-xl-4">
            <div class="card pb-0 info-detail-box">
                ...
                <div class="p-4">
                    <h5 class="report-text gray-text mb-0">{{$info['shipping_name']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['shipping_phone']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['shipping_address']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['shipping_zip']}}</h5>
                    <h5 class="report-text gray-text mb-0">{{$info['shipping_city'].', '. $info['billing_state'] .', '.$info['billing_country']}}</h5>
                </div>
            </div>
        </div>
    </div>
    <div class="row">
        <div class="col-md-12">
            <div class="card pb-0">
                ...
                <div class="row">
                    @php
                        // $totalInvoiceSum=$info->infoTotalInvoiceSum($info['id']);
                        $totalInvoiceSum=0;
                        // $totalInvoice=$info->infoTotalInvoice($info['id']);
                        $totalInvoice= 0;
                    ...
                    <div class="col-md-3 col-sm-6">
                        <div class="p-4">
                            ...
                            <h5 class="report-text mb-3">{{$info['created_at']}}</h5>
                    ...
                    <div class="col-md-3 col-sm-6">
                        <div class="p-4">
                            ...
                            <h5 class="report-text mb-3">{{$info['balance'] ?? 0}}</h5>
                            <h5 class="report-text gray-text mb-0">{{__('Average Sales')}}</h5>
                            <h5 class="report-text mb-0">{{$averageSale ?? 0}}</h5>
                        </div>
                    </div>
                    <div class="col-md-3 col-sm-6">
                        <div class="p-4">
                            <h5 class="report-text gray-text mb-0">{{__('Overdue')}}</h5>
                            <h5 class="report-text mb-3">{{0}}</h5>
                        </div>
                    </div>
                </div>
            </div>
                    ...
                    <div class="table-responsive">
                        <table class="table table-striped mb-0 dataTable">
                            ...
                            @foreach ($proposals as $proposal)
                                <tr>
                                    <td class="Id">
                                        @if(\Auth::guard('info')->check())
                                            <a href="{{ route('info.proposal.show',\Crypt::encrypt($proposal->id)) }}">{{ AUth::user()->proposalNumberFormat($proposal->proposal_id) }}
                                            </a>
                                        @else
                                    ...
                                    @if(Gate::check('edit proposal') || Gate::check('delete proposal') || Gate::check('show proposal'))
                                        <td class="Action">
                                            ...
                                            @can('convert invoice')
                                                ...
                                                @can('show proposal')
                                                    @if(\Auth::guard('info')->check())
                                                        <a href="{{ route('info.proposal.show',$proposal->id) }}" class="edit-icon bg-info" data-toggle="tooltip" data-original-title="{{__('Detail')}}">
                        ...
                        <table class="table table-striped mb-0 dataTable">
                            ...
                            @foreach ($invoices as $invoice)
                                <tr>
                                    <td class="Id">
                                        @if(\Auth::guard('info')->check())
                                            <a href="{{ route('info.invoice.show',\Crypt::encrypt($invoice->id)) }}">{{ AUth::user()->invoiceNumberFormat($invoice->invoice_id) }}
                                            </a>
                                        @else
                                    ...
                                    @if(Gate::check('edit invoice') || Gate::check('delete invoice') || Gate::check('show invoice'))
                                        <td class="Action">
                                            ...
                                            @can('duplicate invoice')
                                                ...
                                                @can('show invoice')
                                                    @if(\Auth::guard('info')->check())
                                                        <a href="{{ route('info.invoice.show',\Crypt::encrypt($invoice->id)) }}" class="edit-icon bg-info" data-toggle="tooltip" data-original-title="{{__('Detail')}}">
                                                        <i class="fas fa-eye"></i>
                                                    </a>
                                                    @else
